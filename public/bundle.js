 ...
 */
var sh = __webpack_require__(8);

console.log(_vuex2.default);
...
console.log(_vuex2.default.state.b.name);
_vuex2.default.dispatch('addAge');
console.log(_vuex2.default.state.b.name);
_vuex2.default.dispatch('increment');
...
var app = new _vue2.default({
    ...
    components: { SetCounter: SetCounter },
...
var moduleA = {
    state: {
        todos: [{ id: '000001', text: '华夏成长混合', type: '混合型' }, { id: '161725', text: '招商中证白酒', type: '股票型' }, { id: '001233', text: '嘉合货币B', type: '货币型' }, { id: '000010', text: '易方达天天理财货币B', type: '货币型' }, { id: '200002', text: '长城久泰沪深300指数', type: '指数型' }]
    },
    getters: {
    ...
    mutations: {
        increment: function increment(state, obj) {
            // setTimeout(function () {
...
var moduleB = {
    ...
    getters: {},
    mutations: {
        addAge: function addAge(state, obj) {
            state.name += obj;
        }
    },
    actions: {
        addAge: function addAge(_ref2) {
...
searchFunc.prototype = {
    init: function init(options) {
        ...
        this._data.target.off().on({
            'input propertychange': function inputPropertychange() {
                var ts = this;
    ...
    addInput: function addInput() {
        ...
        //是否有回调
        if (!this.idEstimate()) str += '<div class="' + this.dom.disId.replace('#', '') + '" id="' + this.dom.disId.replace('#', '') + '" style="display: none;"></div>';
        str += '</div>';
        $(this.opts.searchId).append(str);
    },
    getKeydown: function getKeydown(e) {
        if (this._data.listbox.is(':hidden')) return;
        //是否有回调
        if (this.idEstimate()) return;
    ...
    stringGroup: function stringGroup(data) {
        var str = '',
            other = '';
        var self = this;
        ...
        $.each(data, function (i, v) {
            ...
            $.each(v, function (i, vl) {
                ...
                if (getcdKwd) kcode = kcode.replace(getcdKwd[0], '<em>' + getcdKwd[0] + '</em>');else if (getnmKwd) kname = kname.replace(getnmKwd[0], '<em>' + getnmKwd[0] + '</em>');
    ...
    idEstimate: function idEstimate() {
    ...
    tools: {
        blurPlaceholder: function blurPlaceholder(target) {
            var vl = $(target).val();
        ...
        focusPlaceholder: function focusPlaceholder(target) {
            var vl = $(target).val();
        ...
        keyToName: function keyToName(key) {
            var info = {};
            switch (key) {
                case 'ijjfund':
                    info = {
        ...
        useJSONP: function useJSONP(url, callback) {
            var self = this;
